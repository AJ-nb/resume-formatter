...
- 简历母版与独立岗位版本；创建时保存母版基线，岗位版本不会被自动覆盖
...
首次打开 v2.2 时，当前简历迁移为母版，现有本地版本进入母版历史；原存储键不会删除，并额外保存迁移前回滚副本。每个岗位版本保存独立 `ResumeDocumentV2` 快照、创建时母版基线、JD、证据、要求匹配和导出记录。照片在工作区中按内容哈希去重，导出活动简历时重新补全资产。
...
- Node 单元测试：简历与工作区迁移、岗位快照、资产去重、JD 匹配、证据、投递门禁、选区哈希、事实保护、七类提供商和错误路径
- Playwright：母版与岗位版本、JD、证据生成、工作区备份、投递门禁、导入、编辑、模板、AI 接受 / 撤销、移动端和凭据剥离
